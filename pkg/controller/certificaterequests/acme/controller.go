...
const (
	ControllerName = "acmecertificaterequests"
)
...
func (c *Controller) Register(ctx *controllerpkg.Context) (workqueue.RateLimitingInterface, []cache.InformerSynced, error) {
	...
	orderInformer.Informer().AddEventHandler(&controllerpkg.BlockingEventHandler{
		WorkFunc: controllerpkg.HandleOwnedResourceNamespacedFunc(
			c.log, c.queue, v1alpha1.SchemeGroupVersion.WithKind(v1alpha1.CertificateRequestKind), certificateRequestGetter(c.certificateRequestLister),
		),
	})
