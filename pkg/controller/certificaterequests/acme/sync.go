...
func (c *Controller) Sync(ctx context.Context, cr *v1alpha1.CertificateRequest) (err error) {
	...
	if issuerType != apiutil.IssuerACME {
		c.log.WithValues(
	...
	expectedOrder, err := buildOrder(cr)
	if err != nil {
		// TODO: handle perm/temporary failure
		return err
	}

	// TODO: use a lister
	order, err := c.cmClient.CertmanagerV1alpha1().Orders(expectedOrder.Namespace).Get(expectedOrder.Name, metav1.GetOptions{})
	switch {
	case k8sErrors.IsNotFound(err):
		// TODO: create the Order resource
		_, err := c.cmClient.CertmanagerV1alpha1().Orders(expectedOrder.Namespace).Create(expectedOrder)
		if err != nil {
			return err
		}
		return nil
	...
	if acme.IsFailureState(order.Status.State) {
		// TODO: mark this CSR as failed
		return fmt.Errorf("order is in failure state %q", order.Status.State)
	}

	if order.Status.State == v1alpha1.Valid {
		cr.Status.Certificate = order.Status.Certificate
		c.setCertificateRequestStatus(cr)
		return nil
	}

	if acme.IsFinalState(order.Status.State) {
		// TODO: mark this CSR as failed
		return fmt.Errorf("order is in a non-valid final state %q", order.Status.State)
	}
...
func buildOrder(req *v1alpha1.CertificateRequest) (*v1alpha1.Order, error) {
	block, _ := pem.Decode(req.Spec.CSRPEM)
	if block == nil {
		return nil, fmt.Errorf("failed to decode csr from certificate request resource %s/%s", req.Namespace, req.Name)
	}
	...
	return &v1alpha1.Order{
		ObjectMeta: metav1.ObjectMeta{
			Name:            fmt.Sprintf("%.52s-%d", req.Name, hash),
			Namespace:       req.Namespace,
			Labels:          req.Labels,
			OwnerReferences: []metav1.OwnerReference{*metav1.NewControllerRef(req, v1alpha1.SchemeGroupVersion.WithKind(v1alpha1.CertificateRequestKind))},
		},
		Spec: spec,
	}, nil
}
