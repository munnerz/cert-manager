...
type ClusterIssuerLister interface {
	...
	List(selector labels.Selector) (ret []*certmanager.ClusterIssuer, err error)
	// Get retrieves the ClusterIssuer from the index for a given name.
	Get(name string) (*certmanager.ClusterIssuer, error)
...
// List lists all ClusterIssuers in the indexer.
func (s *clusterIssuerLister) List(selector labels.Selector) (ret []*certmanager.ClusterIssuer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*certmanager.ClusterIssuer))
	})
	return ret, err
}

// Get retrieves the ClusterIssuer from the index for a given name.
func (s *clusterIssuerLister) Get(name string) (*certmanager.ClusterIssuer, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(certmanager.Resource("clusterissuer"), name)
	}
	return obj.(*certmanager.ClusterIssuer), nil
}
